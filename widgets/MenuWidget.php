...
class MenuWidget extends \yii\widgets\Menu
{
     ...
     */
    public $labelTemplate = '<span>{label} {badge}</span>{submenu}';
    /**
     * {@inheritdoc}
     */
    ...
    protected function renderItem($item)
    {
        ...
        if (isset($item['items'])) {
            ...
            $labelTemplate = '<a class="' . ($item['active'] ? 'active' : '') . '" href="{url}">{icon} {label}</a>';
            $linkTemplate = '<a class="' . ($item['active'] ? 'active' : '') . '" href="{url}">{icon} {label}</a>';
        } else {
        ...
        $replacements = [
            ...
            '{icon}' => empty($item['icon']) ? $this->defaultIconHtml
                : '<i class="' . (isset($item['iconType']) ? $item['iconType'] : static::$iconClassType) . ' ' . static::$iconClassPrefix . $item['icon'] . '"></i> ',
